global_input_parameters <- list(
  ...
  x_ic = 0.99, # state-space model JAGS input for the initial phenological state each summer (between 0 and 1)
  tau_ic = 5, # state-space model JAGS input for the precision of our initial x state
  a_ndvi = 3.16, # state-space model JAGS input for gamma shape parameter for ndvi prior
  r_ndvi = 0.316, # state-space model JAGS input for gamma rate parameter for ndvi prior
